...
export const useAriaListbox = ({ id, selectedValue, triggerLabel, isOpen, onSelect, closeOnSelect, onToggle }: ListboxProps) => {
  const controller = useRef(new ListboxController(id));
 
  const triggerProps = useMemo(() => ({
    'aria-label': triggerLabel,
    'aria-haspopup': 'listbox' as HTMLProps<HTMLButtonElement>['aria-haspopup'],
    'aria-expanded': typeof isOpen === 'boolean' ? isOpen : controller.current.getOpen(),
